...
DEFAULT_MODEL_ID = 'jacktol/whisper-large-v3-finetuned-for-ATC'
...
class PipelineInferencer:
    def __init__(self, model_name_or_path: Optional[str] = None, device: Optional[str] = None, batch_size: int = 8) -> None:
        ...
        self.pipe = pipeline('automatic-speech-recognition', model=self.model_name_or_path, tokenizer=self.model_name_or_path, device=self.device, trust_remote_code=True)
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="whisper-large-v3-finetuned-for-ATC inference.")
