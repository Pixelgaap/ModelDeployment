...
DEFAULT_MODEL_ID = 'gravitee-io/bert-small-pii-detection'
...
class PipelineInferencer:
    def __init__(self, model_name_or_path: Optional[str] = None, device: Optional[str] = None, batch_size: int = 8) -> None:
        ...
        self.pipe = pipeline('token-classification', model=self.model_name_or_path, tokenizer=self.model_name_or_path, device=self.device, trust_remote_code=True)
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="bert-small-pii-detection inference.")
