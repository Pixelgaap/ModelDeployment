...
class InferenceRequest(BaseModel):
    image_path: Optional[str] = Field(None, min_length=1, description="Server-local image path. Use image_base64 or image_url for remote API calls.")
    image_base64: Optional[str] = Field(None, min_length=1, description="Base64 image bytes. Data URLs are accepted.")
    image_url: Optional[str] = Field(None, min_length=1, description="HTTP/HTTPS image URL reachable from the server.")
...
app = FastAPI(title='retriever-relik-e5-small-entity-linking-aida-wikipedia-question-encoder API', version="1.0.0", description='FastAPI service for image-feature-extraction.')
...
def infer(request: InferenceRequest) -> InferenceResponse:
    ...
    temp_paths: List[str] = []
    try:
        if any(key in payload for key in ("image_path", "image_base64", "image_url")):
            image_path, should_cleanup = _materialize_remote_file(
